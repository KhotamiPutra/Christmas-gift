...
function endAnim(){
	...
	.to('#buttonContainer', {
		...
		ease: 'power2.out',
		onStart: () => {
			document.querySelector('#buttonContainer').style.pointerEvents = 'auto';
		}
	}, 'message+=1.2')
...
function setMouseMove () {
  ...
  mainSVG.ontouchstart = (e) => {
    ...
    mousePos.x = svgP.x;
    mousePos.y = svgP.y - visibleArea.offset;
  }
  
  mainSVG.ontouchmove = (e) => {
...
function init () {
  ...
  if (nextButton) {
    nextButton.addEventListener('mouseenter', function() {
      gsap.to(this, {
        scale: 1.1,
        boxShadow: '0 12px 30px rgba(255, 159, 28, 0.6)',
        duration: 0.3
      });
    });
    ...
    nextButton.addEventListener('click', function() {
      // Ganti dengan URL halaman bunga kamu
      window.location.href = '../buku/index.html';
    });
  }
